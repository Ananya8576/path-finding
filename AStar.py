...
def search(rows, cols, grid, enemy_pieces, own_pieces, goals):
    ...
    reached=set(begin)
    parent={}
    heurists=initial_h(rows,cols) #setting all h's initially to 0

    h_n=heuristic(goals,begin[0],begin[1],heurists) 
    frontier.append(State(begin,h_n)) #0+h(n)
    parent[begin]= None
    ...
    while len(frontier)>0 and final==False:
        ...
        current=v.pos
        cos=v.cost-heuristic(goals,current[0],current[1],heurists) #cost so far (g(n))
        if current in goals:
            final=True
            tot=cos #just the g(n) as goal h(n)=0
            break
        else:
            for nex in valid_neighbors(board,current,rows,cols):
                ...
                if u not in reached and u not in parent:
                    parent[u]=current
                    h=heuristic(goals,u[0],u[1],heurists) # h(n) for neighbor node
                    g=cos+weight  # the cost so far g(n)
                    heapq.heappush(frontier, State(u,g+h))
                    reached.add(u)    
                else:
                    continue   
    if final==True:
...
def heuristic(goals,r,c,heurists): #chebyshev's distance method applied in chessboard coords
    check=[]
    for g in goals:
        check.append((abs(r-g[0])+ abs(c-g[1]))/2) #averaging over by logic in tut 3
    heurists[r,c]=min(check)
    return heurists[r,c]

def initial_h(rows,cols):
    heurists={}
