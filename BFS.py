...
def search(rows, cols, grid, enemy_pieces, own_pieces, goals):
    if rows == 1 and cols ==1:
        return []
    frontier = Queue()
    begin=own_pieces[0][1] #position tuple for own king
    ...
    while frontier.empty()==False and final==False:
        v=frontier.get()
        if v in goals:
            final=True
            break
        else:
            for u in King(v,rows,cols,board).moves():
    ...
    if final==True:
        path=[]
        while parent[v]!= None:
            succ=(convert_to_char(v[1]),v[0])
            prev=(convert_to_char(parent[v][1]),parent[v][0])
